...
context('First-subject', () => {
  ...
  it('subject button is clickable and renders start button', () => {
    ...
    cy.get('.start').first().should('contain', 'Start');
  });
  ...
  it('start button is clickable and renders copytext', () => {
    ...
    cy.get('.h1').first().should('contain', 'A, b: C.');
  });
  ...
  it('copytext button is clickable and renders where in image', () => {
    cy.get('.input-field').first().type('A, b: C');

    cy.get('.next-button').first().click();

    cy.get('.input-field').first().type('.');

    cy.get('.next-button').first().click();

    cy.get('.next-button').first().click();

    cy.get('.h1').first().should('contain', 'Klikk på laderinngangen');
  });
  ...
  it('where in image button is clickable and renders question for supervisor', () => {
    cy.get('.where-in-picture-img').first().click(50, 50);

    cy.get('.where-in-picture-img').first().click(351, 262);
  ...
  it('question for supervisor button is clickable and renders success screen', () => {
    ...
    cy.get('.h1').first().should('contain', "Du har fullført 'Introduksjon'!");
  });
