...
const WhereInPicture: React.FC<Props> = props => {
  ...
  const checkInput = () => {
    const xInArea = position.x >= props.imageInformation.min.x && position.x <= props.imageInformation.max.x
    const yInArea = position.y >= props.imageInformation.min.y && position.y <= props.imageInformation.max.y
    console.log(xInArea, yInArea)
    if (xInArea && yInArea) {
      // feedback to user
  ...
  return (
    <div>
      ...
      <form
        ...
        <Button classNames='next' onClick={() => onNext()} disabled={!clickedWhileCorrect}>
          Neste
        </Button>
      </form>
    </div>
